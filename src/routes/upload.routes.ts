...
router.get("/files/:id", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const file = await fileService.getFileById(req.params.id); // Returns File with user included
    ...
    if ((file as any).userId !== userId) {
      // Cast to any to access included fields if type definition is strict
...
router.get("/files", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const result = await fileService.getFiles(
      { page, limit },
      {
        filter: { userId: userId }, // Prisma filter uses userId
      }
    );
